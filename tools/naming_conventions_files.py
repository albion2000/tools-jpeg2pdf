...
#should get rid of all forbidden chars for windows for files or dir (same rules)
    r = re.sub(r'[<>:"/\\|?*]', '_', r)            
...
# tries to convert as best as it can anything in ascii characters (special case : '°' becomes 'deg' !). removes accents.
                ...
                for subdir in subdirList :
                    ...
                    if (same_string(stripped,subdir)) : 
                        ...
                        if (1): #not found) : 
                            ...
                            origFullPath = dirName+"\\"+(subdir) # ridoffbadchars
                            printandlog("\n"+strippedFullPath+"  <--  "+ridoffbadchars(subdir));  # ridoffbadchars for avoiding trashing the log file with weird characters.
                            try:
#                                os.rename("\\\\?\\" + re.sub("^\.\\",realrootDir+"\\",origFullPath),"\\\\?\\" + re.sub("^\.\\",realrootDir+"\\",strippedFullPath))
                                os.rename("\\\\?\\" + realrootDir + "\\" + origFullPath[2:],"\\\\?\\" + realrootDir + "\\" + strippedFullPath[2:])
                            except OSError as e:
                                print(f"Code : {e.winerror}")
                                print(f"Message : {e.strerror}")
                                print(f"source : {e.filename}")
                                print(f"destination : {e.filename2}")                            
            ...
            for dirName, subdirList, fileList in os.walk(rootDir):
                ...
                for file in fileList :
                    ...
                    if (same_string(stripped,file)) : 
                        ...
                        printandlog("\n"+dirName+"\\"+stripped+"  <--  "+ridoffbadchars(file)) # to avoid corrupting the log file with weird chars
                        try:
                            os.rename("\\\\?\\" + realrootDir +"\\"+ orig_full_path[2:],"\\\\?\\" + realrootDir +"\\"+ stripped_full_path[2:])
                        except OSError as e:
                            print(f"Code : {e.winerror}")
                            print(f"Message : {e.strerror}")
                            print(f"source : {e.filename}")
